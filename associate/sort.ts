 ...
 * @param associates
 */
export function sortAssociateByFirstNameReversed(
  ...
  associates.sort((a, b) => {
    if (a.associate.lastName.length >= b.associate.lastName.length) {
      for (let i = 0; i < b.associate.lastName.length; i++) {
        let check =
          a.associate.lastName.charCodeAt(i) -
          b.associate.lastName.charCodeAt(i);
        if (check != 0) {
          return check;
        }
      }
 ...
 * @param associates
 */
export function sortAssociateByLastNameReversed(
  associates: AssociateWithFeedback[]
) {
  associates.sort((a, b) => {
    if (a.associate.lastName.length >= b.associate.lastName.length) {
...
export function shuffle(associates: AssociateWithFeedback[]) {
  var currentIndex = associates.length,
    temporaryValue,
    randomIndex;
