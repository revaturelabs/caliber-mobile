...
describe('Quality Audit Component tests', ()=> {
    ...
    test('if the component renders batch dropdowns', ()=>{
        const batchSelection = wrapper.findWhere((node: any) => node.prop('testID') === 'batchSelection').first();
        expect(batchSelection).toExist();
    });
    
    test('if the component renders week tabs', ()=>{
        const weeks = wrapper.findWhere((node: any) => node.prop('testID') === 'weeks').first();
        expect(weeks).toExist();
    });
